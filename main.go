...
func main() {
	...
	//path := fmt.Sprintf(`D:\test99\`)
	//cmd := fmt.Sprintf("git clone %s %s", "http://root:12345678@10.0.0.105/root/testhahaha.git", path)
	//localcmd(cmd)
	syncCode(nblistProject, listp, stoken, susername, dtoken, dusername, oldBmap)
	//pull("D:\\testhaha2", "JdSSPTKxfjvzoBRc1WdH", "root")
	//push("D:\\testhaha2", "JdSSPTKxfjvzoBRc1WdH", "root")
	//Newbranch("D:\\testhaha2", "root", "JdSSPTKxfjvzoBRc1WdH")
	//GetOrginBranch("http://10.0.0.106/api/v4/users", "JdSSPTKxfjvzoBRc1WdH")
	//checkout("D:\\testhaha2", "master")
}
